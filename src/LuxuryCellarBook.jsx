...
const REPORT_STYLES = ["Full Cellar Book", "By Type", "By Region", "By Vintage", "By Price", "Tasting Notes"];
...
function price(wine) {
  return Number(wine.averagePrice ?? wine.estimatedPrice ?? wine.price ?? 0);
}

function qty(wine) {
  return Math.max(1, Number(wine.quantity || 1));
}

function wineType(wine) {
  return wine.category || wine.type || "Other";
}
...
function cleanWine(wine) {
  return {
    ...wine,
    id: wine.id || `${wine.producer}-${wine.wineName}-${wine.vintage}-${wine.slot}`,
    producer: wine.producer || "Producer pending",
    wineName: wine.wineName || wine.name || "Wine name pending",
    category: wineType(wine),
    quantity: qty(wine),
  };
}

function totalBottles(wines) {
  return wines.reduce((sum, wine) => sum + qty(wine), 0);
}

function totalValue(wines) {
  return wines.reduce((sum, wine) => sum + price(wine) * qty(wine), 0);
}

function groupBy(wines, getKey) {
  return Object.entries(wines.reduce((acc, wine) => {
    const key = getKey(wine);
    acc[key] = [...(acc[key] || []), wine];
    return acc;
  }, {})).map(([title, groupWines]) => ({ title, wines: groupWines }));
}
...
function groupByRegion(wines) {
  return groupBy(wines, regionName).sort((a, b) => a.title.localeCompare(b.title));
}

function groupByVintage(wines) {
  return groupBy(wines, (wine) => wine.vintage || "NV").sort((a, b) => {
    if (a.title === "NV") return 1;
    if (b.title === "NV") return -1;
...
function groupByPrice(wines) {
  const bands = [
    { title: "Under $50", test: (n) => n < 50 },
    { title: "$50–$99", test: (n) => n >= 50 && n < 100 },
    { title: "$100–$199", test: (n) => n >= 100 && n < 200 },
    { title: "$200+", test: (n) => n >= 200 },
  ];
...
function Toolbar({ reportStyle, setReportStyle, showPrices, setShowPrices, showNotes, setShowNotes }) {
  return <div className="lux-print-toolbar">
    <div className="lux-toolbar-title"><span>Printable cellar catalogue</span><strong>Cellar Book</strong><small>For cleanest PDF, turn off browser Headers and Footers.</small></div>
    <label>Report<select value={reportStyle} onChange={(e) => setReportStyle(e.target.value)}>{REPORT_STYLES.map((style) => <option key={style}>{style}</option>)}</select></label>
    <label className="lux-check"><input type="checkbox" checked={showPrices} onChange={(e) => setShowPrices(e.target.checked)} /> Prices</label>
    <label className="lux-check"><input type="checkbox" checked={showNotes} onChange={(e) => setShowNotes(e.target.checked)} /> Notes</label>
    <button type="button" onClick={() => window.print()}>Print / Save PDF</button>
  </div>;
}

function RunningHeader() {
  return <header className="lux-running-header"><span>{BRAND_NAME}</span><span>Private Cellar Book</span></header>;
}

function Metric({ label, value }) {
  return <div className="lux-metric"><span>{label}</span><strong>{value}</strong></div>;
}

function SectionTitle({ kicker, title }) {
  return <div className="lux-section-title"><p>{kicker}</p><h2>{title}</h2></div>;
}
...
function SummaryPanel({ title, children }) {
  return <article className="lux-summary-panel"><h3>{title}</h3>{children}</article>;
}

function Line({ label, value }) {
  return <div className="lux-line"><span>{label}</span><strong>{value}</strong></div>;
}
...
export default function LuxuryCellarBook({ wines = [] }) {
  ...
  return <div className="lux-cellar-book-view"><Toolbar reportStyle={reportStyle} setReportStyle={setReportStyle} showPrices={showPrices} setShowPrices={setShowPrices} showNotes={showNotes} setShowNotes={setShowNotes} /><div className="lux-cellar-book"><Cover stats={stats} /><Summary stats={stats} wines={cleanWines} showPrices={showPrices} />{reportStyle === "Full Cellar Book" && <IndexPage wines={cleanWines} showPrices={showPrices} />}<Inventory title={reportStyle === "Full Cellar Book" ? "Primary Inventory by Type" : reportStyle} groups={groups} showPrices={showPrices} showNotes={printNotes} />{reportStyle === "Full Cellar Book" && <><Appendix title="By Region" groups={groupByRegion(cleanWines)} showPrices={showPrices} /><Appendix title="By Vintage" groups={groupByVintage(cleanWines)} showPrices={showPrices} /><Appendix title="By Price" groups={groupByPrice(cleanWines)} showPrices={showPrices} /></>}</div></div>;
}
